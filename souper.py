...
def creating_title_header_page(page_url):
    ...
    links_header = links_header + '<li><a href="#Allowed_HTTP_Verbs">' + cgi.escape(
        "Allowed HTTP Verbs") + '</a> | </li>'
    # Link to Robots.txt
    ...
    links_header = links_header + '<li><a href="#hidden_inputs">' + cgi.escape(
        "<input type='hidden'>") + '</a>  |  </li>'
...
def creating_content(title, content):
    if title == 'Response Header':
        ...
        if not content[0]:
            ...
            page_content = '<tr><td class="e" id="Robots_txt">' + title.encode("utf-8") + ' (<a href=' + content[
                1] + '>' + content[1] + '</a>)</td><td class="v">'
        ...
        page_content = page_content + '<tr><td class="e" id="comments_tags">' + cgi.escape(
            "<!-- -->") + '</td><td class="v">'
        ...
        page_content = page_content + '<tr><td class="e" id="hidden_inputs">' + cgi.escape(
        ...
        # For the title
        page_content = page_content + '<tr><td class="e" id="title_of_page">' + cgi.escape(
            "<title>") + '</td><td class="v">' + content[1].encode('utf-8') + '</td></tr>'
...
def build_web_page(url, cookies_dict, proxies_dict, method):
    ...
    page_content += creating_content('Allowed HTTP Verbs', allowed_verbs)

    # Building Robots.txt Content
    robots_txt_file, base_url_for_robots_txt = get_robots_txt(url)
    page_content += creating_content('Robots.txt', (robots_txt_file, base_url_for_robots_txt))

    # Building All Tags Content
    list_of_all_link_tags, list_of_all_img_tags, list_of_all_comments_tags, list_of_all_meta_tags, list_of_all_hidden_fields = get_all_tags(
        beautiful_response)
    page_content += creating_content('All Tags', (
        list_of_all_link_tags, list_of_all_img_tags, list_of_all_comments_tags, list_of_all_meta_tags,
        list_of_all_hidden_fields))

    # Building All Information Content
    list_of_links, page_title, list_of_emails = get_information(beautiful_response)
    page_content += creating_content('All Information', (list_of_links, page_title, list_of_emails))
